...
void Task_PID_EC(void *pvParameters)  // This is a task.
{
  ...
  // Setup PID EC
  Setpoint_Target_EC = 1.2;
  myPID_EC.SetOutputLimits(-50,50);
  myPID_EC.SetTunings(20,1,0.1); 
  myPID_EC.SetSampleTime (10000) ;
...
//    Serial.print(" | pH value: ");
  ...
  if(millis() - runPIDTime >= 10000) {
    runPIDTime = millis();
    myPID_pH.Compute();
    Serial.print(" => OUTPUT_PH : ");
    Serial.println(Output_pH);
    if(Input_pH<(Setpoint_Target_pH - 0.1) || Input_pH>(Setpoint_Target_pH + 0.1)) {
...
if(millis()-runPIDTime >= 10000) {
    ...
    if(Input_EC<(Setpoint_Target_EC - 0.2) || Input_EC>(Setpoint_Target_EC + 0.2)){
